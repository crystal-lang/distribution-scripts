...
document. In this way it's easy to track the progress of the release (_Helper:
(`distribution-scripts`)
[`scripts/prepare-crystal-release.sh`](./scripts/prepare-crystal-release.sh)_)
...
9. [ ] Ensure that [test-ecosystem](https://github.com/crystal-lang/test-ecosystem) functions and succeeds on master
   - Run [_CI Workflow_](https://github.com/crystal-lang/test-ecosystem/actions/workflows/ci.yml) on `master` (uses nightly build)
...
4. [ ] Smoke test with [test-ecosystem](https://github.com/crystal-lang/test-ecosystem)
   - Run [_CI Workflow_](https://github.com/crystal-lang/test-ecosystem/actions/workflows/ci.yml) with `crystal=branch:release/${VERSION%.*}`.
...
9. [ ] Fast-forward `release/${VERSION%.*}` to `master@${VERSION}`
...
4. [ ] Push changes to OBS for building linux packages
   ...
   6. Now OBS builds the packages. It’s best to follow the build status in the browser:
      ...
      2. `open https://build.opensuse.org/package/show/home:$OBS_USER:branches:devel:languages:crystal/crystal${VERSION%.*}`
   ...
   7. When everything is green, create a submit request against the original
      packages: _Submit package_ link in the page actions (left sidebar) in your branch.
      - (minor) `crystal`
      - `crystal${VERSION%.*}`
...
4. [ ] Merge `release/${VERSION%.*}` branch into `master` (if the two have diverged)
   - This needs to be a _merge commit_. Those are disabled in the GitHub UI.
